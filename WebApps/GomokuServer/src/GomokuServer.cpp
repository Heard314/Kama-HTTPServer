...
GomokuServer::GomokuServer(int port,
    ...
    : httpServer_(port, name, option,true,
                  ssl::SslConfig("../HttpCert/server.crt","../HttpCert/server.key","../HttpCert/server.crt")), maxOnline_(0)
{
    initialize();
}
...
{
    // 初始化数据库连接池
    http::MysqlUtil::init("tcp://172.23.96.1", "admin", "123456", "Gomoku", 1);
    // 初始化会话
    initializeSession();
    // 初始化中间件
