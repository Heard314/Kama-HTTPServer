...
public:
    ...
    HttpServer(int port,
               ...
               bool useSSL = false,
               std::optional<ssl::SslConfig> config = std::nullopt);
    ...
    void sslInit(const ssl::SslConfig& config);
...
private:
    ...
    void onSslMessage(const muduo::net::TcpConnectionPtr &conn,
                                  muduo::net::Buffer *buf,
                                  muduo::Timestamp receiveTime);
    void httpHandler(const muduo::net::TcpConnectionPtr&, const HttpRequest&);

    void requestHandler(const HttpRequest& req, HttpResponse* resp);
